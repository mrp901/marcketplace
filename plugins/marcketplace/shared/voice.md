...
1. **Per-draft recording.** `reply-draft` and `kb-note` record an entry in `state.voice_edits` (ring buffer, max 30) whenever the user's kept or sent version differs from what was drafted. Who performs the comparison differs by handler: `reply-draft` compares on a later dispatch against the same source, by re-reading the thread or sent mail. `kb-note` never compares, because a ticked item dispatches once and it has no second run; `kb-dream` does that comparison for it during its own pass over the note. In both cases the handler's only job at draft time is to append the ledger entry. Each entry: `{tag, register, draft_hash, sent_ref, recorded_at}`.
